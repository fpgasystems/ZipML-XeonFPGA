...
int main(int argc, char* argv[]) {
	...
	if (argc != 4) {
		cout << "Usage: ./app <numFeatures> <miniBatchSize> <f_instances>" << endl;
		return 0;
	}
	else {
	...
	uint32_t starting_numSamples = 131072;
	...
	const uint32_t samplesToTest = 9;

	scd scd_app(1);

	uint32_t index = 0;
	double times[samplesToTest*4];
	for (uint32_t i = 0; i < samplesToTest; i++){
		uint32_t numSamples = starting_numSamples << i;
		cout << "============================================================================== scd_app.numSamples: " << numSamples << endl;
